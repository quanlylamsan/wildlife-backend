...
const WoodActivitySchema = new mongoose.Schema({
  Farm: { // ID của mục gỗ mà bản ghi này thuộc về
    ...
    ref: 'Farm', // Tham chiếu đến model Wood (giả sử bạn có model Wood)
    required: true,
  },
  ...
  source: { // Nguồn gốc (chỉ cho nhập)
    type: String,
    trim: true,
    required: function() { return this.type === 'import'; } // Yêu cầu nếu loại là nhập
  },
  destination: { // Nơi đến (chỉ cho xuất)
    type: String,
    trim: true,
    required: function() { return this.type === 'export'; } // Yêu cầu nếu loại là xuất
  },
